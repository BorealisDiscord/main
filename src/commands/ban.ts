...
export default {
    data: new SlashCommandBuilder()
        ...
        .addUserOption((option) => option.setName('user').setDescription("Who should be banned out of your guild? Pass a(n) mention or user ID here.").setRequired(true))
    ...
    async execute(interaction, client) {
        ...
        if (!member) return interaction.reply({
            content: "I am sorry, but I couldn't find any details related to given member. Double check the user ID or mention of desired user you want to ban.",
            ephemeral: true
        })

        const reason = interaction.options.getString('reason')
        if (!member.bannable) return interaction.reply({
            content: "I am sorry, but I can't ban this user. Perhaps he is already banned / has an administrator permission / is an owner?",
            ephemeral: true
        })
