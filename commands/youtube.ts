...
export default {
    data: new SlashCommandBuilder()
        ...
        .addStringOption((option) => option.setName("keywords").setDescription("Provide keyword to perform a search").setRequired(true)),
    async execute(interaction, client) {
        ...
        const embed = new MessageEmbed()
            ...
            .setTimestamp();
            for (let i = 0; i < 5; i++) {
                embed.addField(`${video[i].title} (${!video[i].isLive ? video[i].duration : "currently live"})`, `Uploaded ${video[i].uploadedAt} by [${video[i].author.name}](${video[i].author.url})\n[Click to watch!](${video[i].url})`);
            };
